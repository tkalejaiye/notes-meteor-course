...
if(Meteor.isClient) {
  ...
  describe('Editor', function() {
    ...
    it('should render note not found', function() {
      const wrapper = mount(<Editor selectedNoteId="testNoteId" browserHistory={browserHistory} call={call}/>);

      expect(wrapper.find('p').text()).toBe('Note Not Found');
    });

    it('should render Choose/Create a note', function() {
      const wrapper = mount(<Editor browserHistory={browserHistory} call={call}/>);

      expect(wrapper.find('p').text()).toBe('Choose/Create a note');
    });
